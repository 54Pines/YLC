...
class Thelegalway_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'thelegalway' );
		$this->widget_id          = 'thelegalway_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'thelegalway' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'thelegalway' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'thelegalway' ),
				'label' => esc_html__( 'Subscribe Title', 'thelegalway' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'thelegalway' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'thelegalway' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'thelegalway' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'thelegalway' ),
				'label' => esc_html__( 'Subscribe submit label', 'thelegalway' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'thelegalway' ),
				'label' => esc_html__( 'Subscribe success', 'thelegalway' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'thelegalway' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'thelegalway' ),
				'label' => esc_html__( 'Follow Title', 'thelegalway' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'thelegalway' ),
			),
			'enable_background' => array(
				...
				'options' => array(
					'enable_background' => array(
						'label' => esc_html__( 'Enable Custom Background', 'thelegalway' ),
			...
			'background_image' => array(
				'type'               => 'media',
				'label'              => esc_html__( 'Background Image', 'thelegalway' ),
				'upload_button_text' => esc_html__( 'Choose Image', 'thelegalway' ),
				'multi_upload'       => false,
			...
			'invert_text_colorscheme' => array(
				...
				'master'  => 'background_image',
				'options' => array(
					'invert_text_colorscheme' => esc_html__( 'Use "Invert scheme" for text color', 'thelegalway' ),
				),
			),
			'background_position' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Position', 'thelegalway' ),
				'value'   => 'center',
				'options' => array(
					'top-left'      => esc_html__( 'Top Left', 'thelegalway' ),
					'top-center'    => esc_html__( 'Top Center', 'thelegalway' ),
					'top-right'     => esc_html__( 'Top Right', 'thelegalway' ),
					'center-left'   => esc_html__( 'Middle Left', 'thelegalway' ),
					'center'        => esc_html__( 'Middle Center', 'thelegalway' ),
					'center-right'  => esc_html__( 'Middle Right', 'thelegalway' ),
					'bottom-left'   => esc_html__( 'Bottom Left', 'thelegalway' ),
					'bottom-center' => esc_html__( 'Bottom Center', 'thelegalway' ),
					'bottom-right'  => esc_html__( 'Bottom Right', 'thelegalway' ),
				),
				'master' => 'background_image',
			),
			'background_repeat' => array(
				...
				'value'   => 'no-repeat',
				'options' => array(
					'repeat'    => esc_html__( 'Repeat', 'thelegalway' ),
					'repeat-x'  => esc_html__( 'Repeat X', 'thelegalway' ),
					'repeat-y'  => esc_html__( 'Repeat Y', 'thelegalway' ),
			...
			'background_size' => array(
				'type'    => 'select',
				'label'   => esc_html__( 'Background Size', 'thelegalway' ),
				'value'   => 'inherit',
				'options' => array(
					'cover'   => esc_html__( 'Cover', 'thelegalway' ),
					'contain' => esc_html__( 'Contain', 'thelegalway' ),
					'auto'    => esc_html__( 'Auto', 'thelegalway' ),
				),
				'master' => 'background_image',
			),
			'background_color' => array(
				'type'   => 'colorpicker',
				'label'  => esc_html__( 'Background Color', 'thelegalway' ),
				'value'  => '#000000',
				'master' => 'background_image',
			),
		);

		add_action( 'wp_ajax_thelegalway_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_thelegalway_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'thelegalway' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'thelegalway' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'thelegalway' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'thelegalway' ),
		);

		$this->nonce = wp_create_nonce( 'thelegalway-subscribe-nonce' );
	...
	public function widget( $args, $instance ) {
		...
		$follow_template            = locate_template( apply_filters( 'thelegalway_subscribe_widget_follow_template_file', 'inc/widgets/subscribe-follow/view/follow-view.php' ), false, false );
		$subscribe_template         = locate_template( apply_filters( 'thelegalway_subscribe_widget_subscribe_template_file', 'inc/widgets/subscribe-follow/view/subcribe-view.php' ), false, false );
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'thelegalway' );
		}
	...
	public function get_social_nav() {
		return thelegalway_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = thelegalway_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'thelegalway-subscribe-nonce' ) ) {
...
add_action( 'widgets_init', 'thelegalway_register_subscribe_follow_widgets' );
/**
 * Register subscribe-follow widget.
 */
function thelegalway_register_subscribe_follow_widgets() {
	register_widget( 'Thelegalway_Subscribe_Follow_Widget' );
}
