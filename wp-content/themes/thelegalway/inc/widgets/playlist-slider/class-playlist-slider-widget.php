...
if ( ! class_exists( 'Thelegalway_Playlist_Slider_Widget' ) ) {

	/**
	 * Class Thelegalway_Playlist_Slider_Widget.
	 */
	class Thelegalway_Playlist_Slider_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name        = esc_html__( 'Playlist Slider', 'thelegalway' );
			$this->widget_description = esc_html__( 'Display playlist slider on your site.', 'thelegalway' );
			$this->widget_id          = apply_filters( 'thelegalway_playlist_slider_widget_ID', 'thelegalway_widget_playlist_slider' );
			$this->widget_cssclass    = apply_filters( 'thelegalway_playlist_slider_widget_cssclass', 'widget-playlist-slider' );
			$this->utility            = thelegalway_utility()->utility;
			$this->settings = array(
				'title' => array(
					'type'  => 'text',
					'value' => esc_html__( 'Playlist Slider', 'thelegalway' ),
				...
				'posts_per_page' => array(
					...
					'max_value' => 50,
					'min_value' => 0,
					'label'     => esc_html__( 'Posts count ( Set 0 to show all. )', 'thelegalway' ),
				),
				'width' => array(
					'type'  => 'text',
					'value' => '100%',
					'label' => esc_html__( 'Slider width ( px, %, rem )', 'thelegalway' ),
				),
				'height' => array(
					'type' => 'text',
					'value' => '500',
					'label' => esc_html__( 'Slider height ( px, rem )', 'thelegalway' ),
				),
				'thumbnail_controls' => array(
					...
					'value'   => true,
					'options' => array(
						false => esc_html__( 'Hide', 'thelegalway' ),
						true  => esc_html__( 'Buttons', 'thelegalway' ),
					),
					'label'       => esc_html__( 'Select Thumbnail Controls', 'thelegalway' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'thelegalway' ),
				),
				'slider_controls' => array(
					...
					'label'       => esc_html__( 'Select Slider Controls', 'thelegalway' ),
					'placeholder' => esc_html__( 'Select Thumbnail Controls', 'thelegalway' ),
				),
				'title_length' => array(
					'type'       => 'stepper',
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Title words length ( Set 0 to hide title. )', 'thelegalway' ),
				),
				'thumb_title_length' => array(
					...
					'min_value'  => '0',
					'step_value' => '1',
					'label'      => esc_html__( 'Thumbnail title words length ( Set 0 to hide title. )', 'thelegalway' ),
				),
				'mate_data' => array(
					'type'  => 'checkbox',
					'value' => array(
					...
					'options' => array(
						'date'     => esc_html__( 'Date', 'thelegalway' ),
						'author'   => esc_html__( 'Author', 'thelegalway' ),
						'comments' => esc_html__( 'Comment count', 'thelegalway' ),
						'category' => esc_html__( 'Category', 'thelegalway' ),
						'post_tag' => esc_html__( 'Tag', 'thelegalway' ),
					),
					'label' => esc_html__( 'Display post meta data', 'thelegalway' ),
				),
				'thumb_mate_data' => array(
					...
					'label' => esc_html__( 'Display thumbnail post meta data', 'thelegalway' ),
				),
			);
			...
			$this->layout_settings = apply_filters( 'thelegalway_playlist_slider_size', array(
		...
		public function the_thumbnail_view() {
			...
			foreach ( $this->posts as $post ) {
				...
				$image = $this->utility->media->get_image( array(
					'size'        => 'thelegalway-thumb-s',
					'mobile_size' => 'thelegalway-thumb-s',
		...
		public function widget( $args, $instance ) {
			...
			if ( $this->posts ) {
				$slider_settings = array(
					...
					'buttons'            => apply_filters( 'thelegalway_playlist_buttons', false ),
...
add_action( 'widgets_init', 'thelegalway_register_playlist_slider_widget' );
/**
 * Register playlist-slider widget.
 */
function thelegalway_register_playlist_slider_widget() {
	register_widget( 'Thelegalway_Playlist_Slider_Widget' );
}
