 ...
 * @return array
 */
function thelegalway_share_buttons( $context = 'loop', $args = array(), $config = array() ) {
	...
	if ( ! thelegalway_is_meta_visible( $meta, $context ) ) {
		return;
	}
	...
	$default_config = apply_filters( 'thelegalway_default_config_share_buttons', array(
	...
	$share_item_html = apply_filters( 'thelegalway_share_button_html',
	...
	foreach ( (array) $networks as $id => $network ) :
		...
		$share_buttons .= sprintf(
			...
			esc_html__( 'Share on ', 'thelegalway' ) . $network['name'],
 ...
 * @return string
 */
function thelegalway_get_single_share_buttons() {
	ob_start();
	thelegalway_share_buttons( 'single' );
